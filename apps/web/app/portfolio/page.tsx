...
export default function PortfolioPage() {
  ...
  const monthlyRoyalty = DEMO_HOLDINGS.reduce((s, h) => s + (h.tokens * h.priceUSD * h.yieldBps / 10000 / 12), 0)
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#F7F9FB', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px' }}>
        <h1 style={{ fontSize: 28, fontWeight: 700, color: '#1C2E45', marginBottom: 32 }}>My Portfolio</h1>

        {!publicKey ? (
          <div style={{ background: '#FFFFFF', borderRadius: 16, padding: 48, textAlign: 'center', border: '0.5px solid #E8EDF2' }}>
            <div style={{ fontSize: 16, color: '#4A6080', marginBottom: 20 }}>Connect your wallet to view your holdings</div>
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>
              {[
                ...
                <div key={s.label} style={{ background: '#FFFFFF', borderRadius: 12, padding: '20px 24px', border: '0.5px solid #E8EDF2' }}>
            ...
            <div style={{ background: '#FFFFFF', borderRadius: 16, border: '0.5px solid #E8EDF2', overflow: 'hidden' }}>
              ...
              {DEMO_HOLDINGS.map(h => (
                <div key={h.license} style={{ padding: '20px 24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 600, color: '#1C2E45', marginBottom: 4 }}>{h.name}</div>
                      <div style={{ fontSize: 12, color: '#4A6080' }}>{h.tokens} tokens · ${h.priceUSD}/token · {h.yieldBps/100}% APY</div>
                      <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#8AACC8', marginTop: 4 }}>{h.assetPDA.slice(0,20)}...</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 20, fontWeight: 700, color: '#1C2E45' }}>${(h.tokens * h.priceUSD).toFixed(2)}</div>
                      <div style={{ fontSize: 12, color: '#3A7A52' }}>+${(h.tokens * h.priceUSD * h.yieldBps / 10000 / 12).toFixed(2)}/mo royalty</div>
                      <span style={{ display: 'inline-block', marginTop: 6, background: '#FAFAE6', color: '#5A5A00', border: '1px solid #B8B840', borderRadius: 12, padding: '2px 10px', fontSize: 11 }}>● {h.warZone} zone</span>
                    </div>
                  </div>
                </div>
              ))}
